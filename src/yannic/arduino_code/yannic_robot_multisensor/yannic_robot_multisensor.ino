...
void read_dual_sensors() {
  ...
  if(measure1.RangeStatus != 4) {     // if not out of range
    nh.loginfo(measure1.RangeMilliMeter);
  ...
  if(measure2.RangeStatus != 4) {
    nh.loginfo(measure2.RangeMilliMeter);
...
pinMode(SHT_LOX1, OUTPUT);
  ...
  range_msg.layout.dim[0].size = 2;
  range_msg.layout.dim[0].stride = 1*2;
  range_msg.layout.data_offset = 0;
  range_msg.data = (int *)malloc(sizeof(int)*2);
